...
export const FeederCard: React.FC<FeederCardProps> = ({ feeder }) => {
  ...
  // Calculate live current state duration dynamically
  const elapsedSec = Math.max(0, Math.floor((now.getTime() - new Date(feeder.lastStatusChange).getTime()) / 1000));
  
  // Total uptime today including currently active seconds
  ...
  const handleTripClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm(language === 'hi' ? `${feeder.hindiName} ?? ????????? ????? (Trip) ?????` : `Trigger Emergency Trip on ${feeder.name}?`)) {
      tripFeeder(feeder.id, 'Emergency / Overcurrent Tripping');
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="p-4 sm:p-5">
          {/* Header row: Feeder name & Live Status Badge */}
            ...
            <div className={`px-2.5 py-1 rounded-full text-xs font-bold flex items-center space-x-1.5 shadow-sm ${
              ...
              {isRunning ? (
                <>
                  ...
                  <span>{language === 'hi' ? '?????? ???? (ON)' : 'RUNNING (ON)'}</span>
                </>
              ) : isTripped ? (
                <>
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />
                  <span>{language === 'hi' ? '??????? (TRIPPED)' : 'TRIPPED'}</span>
                </>
              ) : (
                <>
                  <XCircle className="w-3.5 h-3.5 text-rose-400" />
                  <span>{language === 'hi' ? '??? (OFF)' : 'SHUTDOWN (OFF)'}</span>
                </>
              )}
            </div>
          </div>

          {/* Real-time Dynamic Running / Stopped Timer */}
          <div className="mt-4 p-3 rounded-xl bg-slate-950/70 border border-slate-800/80">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400 flex items-center space-x-1 font-medium">
                ...
                <span>
                  {isRunning 
                    ? (language === 'hi' ? '??????? ??? ?? ??? ??:' : 'Currently Running for:')
                    : (language === 'hi' ? '??????? ??? ??? ??:' : 'Currently Stopped since:')}
                </span>
              </span>
              <span className="text-[11px] text-slate-500">
                {language === 'hi' ? '????? ?????:' : 'Since:'} {formatTime(feeder.lastStatusChange)}
              </span>
            </div>
          ...
          {/* Today's Cumulative Uptime & Downtime Progress */}
          <div className="mt-3 space-y-1.5">
            <div className="flex items-center justify-between text-xs font-mono-scada">
              <span className="text-emerald-400 flex items-center space-x-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                <span>{language === 'hi' ? '?? ???:' : 'Uptime:'} {formatDuration(totalUptimeSec, language)}</span>
              </span>
              <span className="text-rose-400 flex items-center space-x-1">
                <span className="w-1.5 h-1.5 rounded-full bg-rose-400" />
                <span>{language === 'hi' ? '?? ???:' : 'Downtime:'} {formatDuration(totalDowntimeSec, language)}</span>
              </span>
            </div>
          ...
          {/* Live Electrical Telemetry Grid */}
          ...
          {feeder.remarks && (
            <div className="mt-3 flex items-center space-x-1.5 text-[11px] text-slate-400 bg-slate-950/40 px-2.5 py-1.5 rounded-lg border border-slate-800">
              ...
              <span className="truncate" title={feeder.remarks}>
                <strong className="text-slate-300">{language === 'hi' ? '???????:' : 'Note:'}</strong> {feeder.remarks}
              </span>
            </div>
          )}

          {/* Action Control Row for Operator */}
          {role === 'operator' ? (
            <div className="mt-4 pt-3 border-t border-slate-800 flex items-center space-x-2">
              ...
              >
                ...
                <span>
                  {isRunning
                    ? (language === 'hi' ? '???? ??? ???? (OFF)' : 'SWITCH OFF FEEDER')
                    : (language === 'hi' ? '???? ???? ???? (ON)' : 'SWITCH ON FEEDER')}
                </span>
              </button>
              ...
              {isRunning && (
                <button
                  ...
                  title={language === 'hi' ? '????? ????????' : 'Trip Relay Simulation'}
                >
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 text-center">
              <span className="text-[11px] text-indigo-300/80 font-medium flex items-center justify-center space-x-1">
                ...
                <span>{language === 'hi' ? '??????? ????????? ??????? (???? ?????)' : 'Officer Monitoring Stream (Live)'}</span>
              </span>
            </div>
          )}
